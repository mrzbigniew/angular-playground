...
import { Component, OnInit, Input, TemplateRef, ViewChild, ElementRef, AfterViewInit, NgZone, ChangeDetectorRef, OnDestroy, ContentChild } from '@angular/core';
...
export class LazyListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ContentChild(LazyListItemTemplateDirective, {read: TemplateRef}) itemTemplate: TemplateRef<HTMLElement>;
  ...
  @Input() set items(items: LazyListItem[]) {
    if (items) {
      this.allItems = items;
      this.itemsToDisplay = this.allItems.slice(0, this.initItemsCount);
      this.scrollToTop();
    }
  }
  ...
  private addChunks() {
    this.allItems.slice(this.itemsToDisplay.length, this.lazyItemsChunkCount).forEach(
